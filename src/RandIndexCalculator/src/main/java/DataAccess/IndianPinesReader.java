...
class IndianPinesReader extends DatasetReader {
    ...
    public void readDataset() throws IOException {
        try(BufferedReader reader = new BufferedReader(new FileReader(file))){
            ...
            final Pattern spatialPattern = Pattern.compile("(\\d+),");
            final int spatialGroup = 1;
            final int classGroup = 3;

            String line;
            for(int i = 0; (line = reader.readLine()) != null; ++i){
                if(i > 1){ //skip the first two lines
                    dataset.put(parseSpatialAttrs(line, spatialPattern, spatialGroup),
                            parseBelongingClass(line, classPattern, classGroup));
                }
            }
        }
    }
    ...
    public static void main(String[] args) throws IOException {
        DatasetReader reader = new IndianPinesReader("/home/data/Dropbox/Tesi/CLUSTERING/TestDatasets/Indian_Pine_DataSet.csv");
